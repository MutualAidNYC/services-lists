...
import { Heading, Stack, Box, Center, HStack, Text, Tooltip, Flex, VStack, LinkBox, LinkOverlay } from '@chakra-ui/react'
            ...
            <ServiceListProvider value={serviceListHandler}>
                <HStack display={{ base: 'none', md: 'inherit' }} w='100%' minH='calc(100vh - 96px)' h='100%' spacing={0} position='relative' >
                    <VStack w='50%' minH='calc(100vh - 96px)' h='100%' >
                        <Heading size='3xl' pt={4} pb={4} textAlign='center' >{listName}</Heading>
                ...
                <Box display={{ base: 'inherit', md: 'none' }} w='100%' minH='calc(100vh - 96px)' h='100%' >
                    ...
                    <VStack bottom={2} height='180px' w='100%' pos='absolute' >
                        <HStack overflow='scroll' w='100%' h='100%'>
                            {!isLoading && services.map((service) =>
                                <Box key={service.id} onClick={() => setSelectedAddress(getAddress(service))} cursor='pointer' minW='200' h='100%' onTouchStart={() => { }}
                                    bg='teal.700' color='white' rounded='lg' overflow='scroll'>
                                    <LinkBox >
