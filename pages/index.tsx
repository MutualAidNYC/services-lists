    ...
    <Box w='100%' mb={8} display='flex' flexDirection='column'>
      ...
      <VStack w='100%' mt={8} px={{ base: '4px', sm: '8px', md: '16px' }} spacing={0}>
        ...
        <Stack w='100%' justifyContent='space-between' direction='row'>
          <Box />
          <Text fontWeight='bold' > {((displayAmount[1] + 1) - (displayAmount[0])) + '  results loaded.'}  </Text>
          <Text fontSize='md' pr={{ base: '12px' }}> {(displayAmount[0] + 1) + ' ... ' + (displayAmount[1] + 1)} </Text>
        </Stack>


        <HStack alignItems='left' justifyContent='right' w='100%'>
          <ChevronLeftIcon w='32px' h='32px' _hover={{ width: '40px', height: '40px' }} onClick={() => {
            displayAmount[0] - 10 >= 0 ? setDisplayAmount([displayAmount[0] - 10, displayAmount[1] - 10])
              : null
          }} />

          <ChevronRightIcon w='32px' h='32px' _hover={{ width: '40px', height: '40px' }} onClick={() => {
            displayAmount[1] + 10 <= servicesLists?.length ? setDisplayAmount([displayAmount[0] + 10, displayAmount[1] + 10])
              : null
          }} />
        </HStack>
      </VStack>
