  ...
  return (
    <Box w='100%' mb={8} display='flex' flexDirection='column'>
      <VStack w='100%' mb={8} alignItems='left' pl={{ base: 4, sm: 16, md: 32 }} >
        <Heading fontSize={{ base: '4xl', sm: '5xl' }} mt={8} mb={4}>Services Lists</Heading>
            ...
            <>
              {
                list.map((item, i) => {
                  return (
                    <Stack alignItems='left' key={i} w='100%' maxW='4xl' pl={{ base: 4, sm: 16, md: 32 }} >
                      <ServicesListItem
                        pr={{ base: '2px', sm: '8px' }}
                        w='100%'
              ...
              <Text textAlign='center' fontWeight='light' > {list.length == 1 ? ` Showing ${list.length} out of ${servicesLists.length} results.` : ` Showing ${list.length} out of ${servicesLists.length} results.`}  </Text>
